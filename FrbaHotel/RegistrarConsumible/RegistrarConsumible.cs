...
namespace FrbaHotel.RegistrarConsumible
{
    public partial class RegistrarConsumible : Form
    {
        ...
        private void obtenerRegimenes()
        {
            ...
            SqlCommand query = new SqlCommand("SELECT b.DESCRIPCION, LOS_MAGIOS.PRECIO_HABITACION("+
                "(SELECT R.PRECIO_DOLARES FROM LOS_MAGIOS.REGIMENES R, LOS_MAGIOS.RESERVAS c WHERE c.CODIGO_RESERVA=@codReserva AND R.CODIGO_REGIMEN = c.CODIGO_REGIMEN)," +
                "(SELECT TH.CANTIDAD_PERSONAS FROM LOS_MAGIOS.TIPOS_HABITACION TH, LOS_MAGIOS.HABITACIONES_POR_RESERVA HR, LOS_MAGIOS.HABITACIONES H WHERE HR.CODIGO_RESERVA = @codReserva AND HR.ID_HOTEL = H.ID_HOTEL AND HR.NUMERO_HABITACION = H.NUMERO_HABITACION AND H.CODIGO_TIPO_HABITACION = TH.CODIGO_TIPO_HABITACION)," +
                "(SELECT H.ESTRELLAS FROM LOS_MAGIOS.HOTELES H, LOS_MAGIOS.HABITACIONES_POR_RESERVA HR, LOS_MAGIOS.RESERVAS RE "+
                "WHERE RE.CODIGO_RESERVA=@codReserva AND HR.CODIGO_RESERVA = RE.CODIGO_RESERVA AND HR.ID_HOTEL = H.ID_HOTEL)),(SELECT DATEDIFF(day, FECHA_INGRESO, FECHA_EGRESO)+1 FROM LOS_MAGIOS.ESTADIAS WHERE CODIGO_RESERVA =@codReserva), b.CODIGO_REGIMEN FROM LOS_MAGIOS.REGIMENES b, LOS_MAGIOS.RESERVAS c WHERE c.CODIGO_RESERVA=@codReserva AND b.CODIGO_REGIMEN = c.CODIGO_REGIMEN", baseDeDatos);
