...
namespace FrbaHotel
{
    public class rolesDeUsuario
    {
        ...
        public DataTable listarRoles(string usuario) 
        {
            ...
            DataTable Tabla = new DataTable();
            comando.Connection = unaConexion.abrirConexion();
            comando.CommandText = "SELECT A.ID_ROL AS 'ID_ROL', A.NOMBRE AS 'NOMBRE',E.NOMBRE AS 'NOMBRE_HOTEL' FROM LOS_MAGIOS.ROLES A INNER JOIN LOS_MAGIOS.ROLES_POR_USUARIO B ON A.ID_ROL = B.ID_ROL INNER JOIN LOS_MAGIOS.USUARIOS C ON B.USUARIO = C.USUARIO INNER JOIN LOS_MAGIOS.HOTELES_POR_USUARIOS D ON C.USUARIO = D.USUARIO INNER JOIN LOS_MAGIOS.HOTELES E ON D.ID_HOTEL = E.ID_HOTEL  WHERE C.NOMBRE = '"+usuario + "'";
            LeerFilas = comando.ExecuteReader();
            Tabla.Load(LeerFilas);
            LeerFilas.Close();
            unaConexion.cerrarConexion();
